...
from app.api.v1.schemas import AnalyzeRequest, AnalyzeResponse, AnalysisMetrics, Issue, FileInput
...
class TestAnalyzer:
    ...
    async def analyze_files(
        ...
        logger.info(f"Starting analysis {analysis_id} with mode: {mode}, files: {len(files)}")
        ...
        try:
            ...
            logger.info(f"Analysis {analysis_id} completed: {len(all_issues)} issues found in {metrics.analysis_time_ms}ms")
    ...
    async def _parse_files_parallel(self, files: List[FileInput]) -> List[ParsedTestFile]:
        ...
        for i, result in enumerate(parsed_files):
            if isinstance(result, Exception):
                ...
                logger.warning(f"File {files[i].path} has syntax errors: {result.syntax_error_message}")
    ...
    async def _run_rule_analysis(self, parsed_files: List[ParsedTestFile]) -> List[Issue]:
        ...
        for parsed_file in parsed_files:
            try:
                ...
                logger.error(f"Rule engine failed for file {parsed_file.file_path}: {e}")
        
        return rule_issues
    
    async def _run_llm_analysis(self, parsed_files: List[ParsedTestFile], mode: str) -> List[Issue]:
    ...
    async def _analyze_file_with_llm(self, parsed_file: ParsedTestFile) -> List[Issue]:
        ...
        for test_class in parsed_file.test_classes:
            for test_method in test_class.methods:
                method_issues = await self._analyze_function_with_llm(test_method, parsed_file)
    ...
    def _merge_issues(self, rule_issues: List[Issue], llm_issues: List[Issue], mode: str) -> List[Issue]:
    ...
    def _is_duplicate_issue(self, issue: Issue, existing_issues: List[Issue]) -> bool:
        ...
        for existing in existing_issues:
            ...
            if (issue.file == existing.file and 
                issue.line == existing.line and
                issue.type.split("-")[-1] == existing.type.split("-")[-1]):
                return True
        
        return False
    
    def _calculate_metrics(self, parsed_files: List[ParsedTestFile], issues: List[Issue], start_time: float) -> AnalysisMetrics:
