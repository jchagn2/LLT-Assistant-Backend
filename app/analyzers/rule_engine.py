...
class Rule(ABC):
    ...
    def create_issue(
        ...
        return Issue(
            ...
            suggestion=suggestion or IssueSuggestion(
                action="remove",
                explanation="No specific suggestion provided"
            )
        )
...
class RedundantAssertionRule(Rule):
    ...
    def _check_function(self, test_func: TestFunctionInfo, file_path: str) -> List[Issue]:
...
class MissingAssertionRule(Rule):
    ...
    def check(self, parsed_file: ParsedTestFile) -> List[Issue]:
        ...
        for test_func in parsed_file.test_functions:
            if self._has_no_assertions(test_func):
                issues.append(self._create_missing_assertion_issue(test_func, parsed_file.file_path))
        
        # Check test class methods
        for test_class in parsed_file.test_classes:
            for test_method in test_class.methods:
                if self._has_no_assertions(test_method):
                    issues.append(self._create_missing_assertion_issue(test_method, parsed_file.file_path))
    ...
    def _create_missing_assertion_issue(self, test_func: TestFunctionInfo, file_path: str) -> Issue:
...
class UnusedVariableRule(Rule):
    ...
    def check(self, parsed_file: ParsedTestFile) -> List[Issue]:
        ...
        # Check module-level test functions
        for test_func in parsed_file.test_functions:
            issues.extend(self._check_function_variables(test_func, parsed_file.file_path))
        
        # Check test class methods
        for test_class in parsed_file.test_classes:
            for test_method in test_class.methods:
                issues.extend(self._check_function_variables(test_method, parsed_file.file_path))
        
        return issues
    
    def _check_function_variables(self, test_func: TestFunctionInfo, file_path: str) -> List[Issue]:
        ...
        try:
            ...
            if not func_ast.body or not isinstance(func_ast.body[0], (ast.FunctionDef, ast.AsyncFunctionDef)):
                return issues
